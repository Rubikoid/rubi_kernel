...
tar -xJf binutils-2.36.tar.xz
tar -xJf gcc-11.1.0.tar.xz
```

```bash
export PREFIX="$HOME/gcc_cross"
...
mkdir build-gcc
cd build-gcc
../gcc-11.1.0/configure --target=$TARGET --prefix="$PREFIX" --disable-nls --enable-languages=c,d,c++ --without-headers --disable-libssp --disable-libgomp --disable-libmudflap --disable-bootstrap --disable-multilib
make all-gcc -j 16
